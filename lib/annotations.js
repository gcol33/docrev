...
export function stripAnnotations(text, options = {}) {
  ...
  do {
    ...
    // Also clean up adjacent spaces: "word {--deleted--} next" → "word next" (not "word  next")
    text = text.replace(new RegExp(' ?' + PATTERNS.delete.source + ' ?', 'gs'), (match) => {
      // If deletion was between two spaces, leave one space
      // If deletion was at start/end of line, leave nothing
      const hadSpaceBefore = match.startsWith(' ');
      const hadSpaceAfter = match.endsWith(' ');
      return (hadSpaceBefore && hadSpaceAfter) ? ' ' : '';
    });
