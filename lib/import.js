...
export async function extractFromWord(docxPath, options = {}) {
  ...
  try {
    ...
    // Handle pandoc comment patterns
    // IMPORTANT: Pandoc outputs comments as: [COMMENT_TEXT]{.comment-start id="N" ...}ANCHOR_TEXT[]{.comment-end id="N"}
    // The [COMMENT_TEXT] is the actual reviewer comment (should be removed from body)
    // The ANCHOR_TEXT (between .comment-start and .comment-end) is the text being commented on
    // The actual comment content is in comments.xml and inserted separately via extractWordComments
    // So we need to REMOVE the [COMMENT_TEXT]{.comment-start ...} completely, keeping only ANCHOR_TEXT
