 ...
 */
/**
 * Extract the sentence containing the anchor text
 * @param {string} text - Full document text
 * @param {string} anchor - Anchor text to find
 * @returns {{sentence: string, anchorPos: number} | null}
 */
function getSentenceContext(text, anchor) {
  const idx = text.indexOf(anchor);
  if (idx === -1) return null;

  // Find sentence boundaries (., !, ?, or paragraph break)
  const sentenceEnd = /[.!?]\s|[\n\r]{2,}/;

  // Find start of sentence (look backwards for sentence end or start of text)
  let start = idx;
  ...
  const afterAnchor = text.slice(end);
  const endMatch = afterAnchor.match(sentenceEnd);
  if (endMatch) {
    end = end + endMatch.index + 1;
  } else {
  ...
  return {
    ...
    anchorPos: idx - start // position of anchor within sentence
  };
}
...
export function insertCommentsIntoMarkdown(markdown, comments, anchors) {
  ...
  const commentsWithPositions = comments.map((c) => {
    ...
    // Strategy: Find the full sentence containing the anchor for unique matching
    // This is 99.9% accurate since sentences are almost always unique

    // First, try to find the anchor text directly
    ...
    if (occurrences > 1) {
      duplicateWarnings.push(`"${anchor.slice(0, 40)}..." appears ${occurrences} times`);
    }

    // Find position - use first occurrence (usually correct in sequential docs)
    let pos = -1;
    const directIdx = resultLower.indexOf(anchorLower);
    if (directIdx !== -1) {
      pos = directIdx + anchor.length;
    }
    ...
    if (pos === -1) {
      ...
      if (normalizedIdx !== -1) {
        ...
        const beforeNormalized = normalizedResult.slice(0, normalizedIdx + normalizedAnchor.length);
        pos = beforeNormalized.length;
      } else {
        unmatchedCount++;
      }
    }
  ...
  if (duplicateWarnings.length > 0) {
    console.warn(`Warning: Duplicate anchor text found (comment placement may be imprecise):`);
