...
export function generateResponseLetter(comments, options = {}) {
    const { title = 'Response to Reviewers', authorName = 'Author', includeContext = true, includeLocation = true, } = options;
    const lines = [];
    lines.push(`# ${title}`);
    lines.push('');
    ...
    for (const reviewer of reviewers) {
        ...
        if (reviewer.toLowerCase() === authorName.toLowerCase())
            continue;
        if (reviewer.toLowerCase() === 'claude')
            continue;
    ...
    // Summary stats
    const totalComments = comments.filter(c => !c.author.toLowerCase().includes('claude') &&
        c.author.toLowerCase() !== authorName.toLowerCase()).length;
