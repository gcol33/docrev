 ...
 */
import { chalk, fs, fmt, loadBuildConfig, } from './context.js';
/**
...
export function register(program) {
    ...
    program
        ...
        const { isGitRepo, getDefaultBranch, getCurrentBranch, getChangedFiles, getWordCountDiff, compareFileVersions, } = await import('../git.js');
        if (!isGitRepo()) {
        ...
        if (options.files) {
            ...
            filesToCompare = fs.readdirSync('.').filter(f => f.endsWith('.md') && !['README.md', 'CLAUDE.md'].includes(f));
        }
    ...
    program
        ...
        const { isGitRepo, getFileHistory, getRecentCommits, hasUncommittedChanges, } = await import('../git.js');
        if (!isGitRepo()) {
            console.error(fmt.status('error', 'Not a git repository'));
            process.exit(1);
        }
