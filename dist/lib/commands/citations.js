...
export function register(program) {
    ...
    program
        ...
        if (!mdFiles || mdFiles.length === 0) {
            mdFiles = fs.readdirSync('.').filter(f => f.endsWith('.md') && !['README.md', 'CLAUDE.md'].includes(f));
        }
    ...
    program
        ...
        for (const [label] of registry.figures) {
            if (!refCounts.get(`fig:${label}`))
                unreferenced.push(`@fig:${label}`);
        }
        for (const [label] of registry.tables) {
            if (!refCounts.get(`tbl:${label}`))
                unreferenced.push(`@tbl:${label}`);
        }
    ...
    program
        ...
        if (action === 'from-word') {
            ...
            const mdFiles = fs.readdirSync('.').filter(f => f.endsWith('.md') && !['README.md', 'CLAUDE.md'].includes(f));
    ...
    program
        ...
        const { extractPdfComments, extractPdfAnnotationsWithText, formatPdfComments, getPdfCommentStats, insertPdfCommentsIntoMarkdown, formatAnnotationWithText, } = await import('../pdf-import.js');
