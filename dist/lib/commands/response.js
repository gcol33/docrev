 ...
 */
import { chalk, fs, path, fmt, collectComments, generateResponseLetter, groupByReviewer, getUserName, } from './context.js';
/**
...
export function register(program) {
    ...
    program
        ...
        if (!mdFiles || mdFiles.length === 0) {
            const allFiles = fs.readdirSync('.').filter(f => f.endsWith('.md') && !['README.md', 'CLAUDE.md', 'paper.md'].includes(f));
            mdFiles = allFiles;
        }
        if (mdFiles.length === 0) {
        ...
        const reviewers = [...grouped.keys()].filter(r => !r.toLowerCase().includes('claude') &&
