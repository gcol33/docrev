 ...
 */
import { chalk, fs, path, fmt, jsonMode, jsonOutput, findFiles, stripAnnotations, countAnnotations, getComments, interactiveReview, exitWithError, getFileNotFoundSuggestions, requireFile, } from './context.js';
/**
...
export function register(program) {
    ...
    program
        ...
        if (file) {
            ...
            const rows = [];
            if (counts.inserts > 0)
                rows.push([chalk.green('+'), 'Insertions', chalk.green(counts.inserts.toString())]);
            if (counts.deletes > 0)
                rows.push([chalk.red('-'), 'Deletions', chalk.red(counts.deletes.toString())]);
            if (counts.substitutes > 0)
                rows.push([chalk.yellow('~'), 'Substitutions', chalk.yellow(counts.substitutes.toString())]);
            if (counts.comments > 0)
                rows.push([chalk.blue('#'), 'Comments', chalk.blue(counts.comments.toString())]);
        ...
        if (totalChanges > 0 || totalComments > 0) {
            ...
            if (rows.length > 0) {
                console.log(fmt.table(['File', 'Words', 'Ins', 'Del', 'Sub', 'Cmt'], rows, { align: ['left', 'right', 'right', 'right', 'right', 'right'] }));
            }
        }
