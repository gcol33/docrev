 ...
 */
function hasNumberedAffiliations(config) {
    if (!config.affiliations || Object.keys(config.affiliations).length === 0)
        return false;
 ...
 */
function generateLatexAuthorBlock(config) {
    const lines = [];
 ...
 */
function generateMarkdownAuthorBlock(config) {
    const lines = [];
    // Map affiliation keys to numbers
    const affiliationKeys = Object.keys(config.affiliations);
    const keyToNum = new Map();
 ...
 */
export function applyFormatTransforms(content, format, config, registry) {
    if (format === 'pdf' || format === 'tex') {
 ...
 */
export function prepareForFormat(paperPath, format, config, _options = {}) {
 ...
 */
export function resolveOutputDir(directory, config) {
