...
export function listCustomProfiles() {
    ...
    if (fs.existsSync(USER_PLUGINS_DIR)) {
        const files = fs.readdirSync(USER_PLUGINS_DIR).filter(f => f.endsWith('.yaml') || f.endsWith('.yml') || f.endsWith('.json'));
        for (const file of files) {
            try {
    ...
    if (fs.existsSync(PROJECT_PLUGINS_DIR)) {
        const files = fs.readdirSync(PROJECT_PLUGINS_DIR).filter(f => f.endsWith('.yaml') || f.endsWith('.yml') || f.endsWith('.json'));
        for (const file of files) {
            try {
